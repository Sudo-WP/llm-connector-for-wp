...
**LLM Connector for WordPress** is a read-only MCP (Model Context Protocol) bridge for WordPress. It exposes a REST API that AI clients authenticate to with a per-plugin API key and use to fetch site diagnostics — plugin/theme inventories, system status, user counts, content statistics. Verified against the Claude.ai web UI, Claude Code, and Gemini CLI; compatible with Cursor, Windsurf, and VS Code (Cline) via the auto-generated MCP config. Current in-development version: **0.4.0-dev** (last shipped: **0.3.0**). See [README.md](README.md) for the full feature tour and [SECURITY.md](SECURITY.md) for the security model.
...
includes/
  ...
  Security/
    ...
    Write_Permission_Manager.php # Write-tier permission scaffold (0.4.0-dev). NO write endpoints yet
  Admin/
    Admin_Interface.php       # Settings page, API key management, "Connect Your AI Client"
                              # MCP config generator, Access Log page renderer, AJAX handlers
